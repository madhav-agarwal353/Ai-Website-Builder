...
Return ONLY the enhanced prompt, nothing else. Make it detailed but concise (2-3 paragraphs max)`
        ...
        const enhance_prompt = enhance_prompt_response.choices[0].message.content;

        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: `I have enhance your prompt to : ${enhance_prompt}`,
                projectId: project.id
            }
        })
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: `now generating ypur website...`,
                projectId: project.id
            }
        })
