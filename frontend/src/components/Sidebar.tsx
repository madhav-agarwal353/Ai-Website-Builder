...
interface SideBarProps {
    isMenuOpen: boolean
    project: Project
    isBusy: boolean // parent tells us if any generation is happening
...
const Sidebar = ({
    ...
    const handleRollbackClick = (versionId: string) => {
        // Parent handles confirmation & API; we simply forward the call.
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isBusy) return; // disabled while any generation is ongoing

        try {
            await onGenerate(input);
            setinput('');
        } catch (err) {
            // parent already handles toasts, but catch to avoid unhandled rejections
    ...
    }, [project.conversation.length, project.versions.length, isBusy])
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between">
                                        {isCurrent ? (
                                            ...
                                            <button
                                                onClick={() => handleRollbackClick(ver.id)}
